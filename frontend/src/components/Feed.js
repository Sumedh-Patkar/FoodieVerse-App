...
const Feed = ({apiServer}) => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                const response = await axios.get('http://' + apiServer + '/api/feed/');
            ...
            {posts.map((post, index) => (
                <div className="col">
                    <div className="card p-3" key={post.id}>
                        <img src={require('../assets/images/food-bg' + ((index%12)+1) + '.jpg')} className="card-img-top pb-3" alt="..."></img>
